...
(x_test,y_test) = loaddata(r"killqueen\test.pickle")
...
#     if(y_test[i] != pre[i]):
#         img = cv2.resize(x_test[i],(500,500),interpolation=cv2.INTER_AREA)
#         cv2.imshow(f"{lable[pre[i]]}is not{lable[y_test[i]]}",img)
#         cv2.waitKey(0)

# for i in range(len(pre)):
#     if(pre[i] != y_test[i]):
#         cv2.imshow(f"{pre[i]}",x_test[i])
#         cv2.waitKey(0)    
